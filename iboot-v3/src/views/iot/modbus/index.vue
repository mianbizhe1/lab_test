  ...
  <UView name="设备">
    <UViewSearch v-model="searchModel">
      <URow col="search">
        ...
        <USelectItem field="productId" label="所属产品" @change="productChange" :allowClear="true"
                     url="/iot/modbus/products?deviceType=Gateway" labelField="name" valueField="id" />
        ...
        <UButton func="query" url="/iot/modbus/view?deviceType=Gateway" ref="searchRef">搜索</UButton>
        <UButton func="reset">重置</UButton>
        <UButton func="add">新增</UButton>
      </URow>
    </UViewSearch>
    <UViewModal title="Modbus设备管理" :span="[8, 16]" v-model="editModel" :rules="rules" width="680px" @edit="gatewayEdit">
      <template #default="{model}">
        <URow :responsive="2">
          ...
          <USelectItem field="productId" label="所属产品" @change="productChange" v-model:source="productSource"
                       url="/iot/modbus/products?deviceType=Gateway" labelField="name" valueField="id" :disabled="model.id != null">
          </USelectItem>
          <UAutoCompleteItem v-if="modbusType == 'RTU'" field="deviceSn" label="串口" :maxlength="32" url="/iot/serial/available" />
          ...
          <a-divider v-if="protocolConfig && protocolConfig.length > 0" style="margin: 0px 0px 12px" dashed>网关设备配置(重连客户端后生效)</a-divider>
    ...
    <UViewTable :columns="columns" :scroll="{x: 1230}">
      ...
      <template #action="{record}">
        <UFuncTag func="edit" :data="record" url="/iot/device/edit" v-auth="'iot:modbus:edit'">修改</UFuncTag>
        <UTag func="open" uid="child" :data="record" color="#2db7f5" v-auth="'iot:modbus:child'">子设备</UTag>
        <UTag func="ajax" :disabled="record['connectType'] != 'Client'" :data="record" color="#55acee"
              v-auth="'iot:modbus:connect'" :url="'/iot/device/connect/'+(record.status=='online' ? 'offline' : 'online')">
          {{record.status=='online' ? '断开' : '连接'}}
        </UTag>
        <UFuncTag func="del" :data="record" url="/iot/modbus/del" v-auth="'iot:modbus:del'">删除</UFuncTag>
      </template>
    </UViewTable>
    <UDrawer uid="child" placement="top" :title="'子设备管理('+editModel.name+')'" @open="childOpen" height="100%">
      <ULinkView uid="childView">
        <UViewSearch v-model="childSearchModel">
          <URow col="search">
            <UInputItem field="name" label="设备名称" />
            <UInputItem field="deviceSn" label="从机地址" />
        ...
        <UViewModal title="子设备管理" :span="[8, 16]" v-model="childModel" :rules="childRules" width="680px" @edit="childEdit">
          <template #default="{model}">
            <URow :responsive="2">
              <UInputItem field="name" label="设备名称" :maxlength="32" />
              <UInputNumberItem field="deviceSn" label="从机地址" :maxlength="3" :min="0" :max="247" :step="1" />
              <USelectItem field="productId" label="所属产品" @change="productChange"
                           url="/iot/modbus/products?deviceType=Child" labelField="name" valueField="id">
              </USelectItem>
...
export default {
  ...
  setup() {
    ...
    let childColumns = ref([
      ...
      {field: "deviceSn", title: "从机地址", width: 80},
    ...
    let childRules = reactive({
        ...
        status: {required: true, message: '设备状态必填'},
        deviceSn: {required: true, message: '从机地址必填'},
        productId: {required: true, message: '所属产品必填'},
        productTypeId: {required: true, message: '产品类型必填'},
    })
    ...
    return {columns, rules, editModel, searchModel, selectedProduct, productSource, searchRef, protocolConfig, mapOpen
      , centerPoint, childSearchModel, childModel, childColumns, childRules, childProtocolConfig, mapOpenType, dataFormat, modbusType}
  },
  methods: {
    productChange(value, model) {
      this.selectedProduct = model;
      this.editModel['config'] = {};
      if(model['protocolCode'] == 'MODBUS_TCP') {
        this.modbusType = "TCP";
      } else {
        this.modbusType = "RTU";
      }
    ...
    gatewayEdit(editModel) {
      if(!editModel.id) {
        ...
        let product = this.productSource[editModel.productId];
        if(product) {
          if(product['protocolCode'] == 'MODBUS_TCP') {
            this.modbusType = "TCP";
          } else {
            this.modbusType = "RTU";
          }
