    ...
    <UViewSearch v-model="searchModel">
      <URow col="search">
        ...
        <USelectItem field="productId" label="所属产品" @change="productChange" :allowClear="true"
                     url="/iot/mqtt/products?deviceType=Gateway" labelField="name" valueField="id" />
        ...
        <UButton func="query" url="/iot/mqtt/view?deviceType=Gateway" ref="searchRef">搜索</UButton>
        <UButton func="reset">重置</UButton>
        <UButton func="add">新增</UButton>
      </URow>
    </UViewSearch>
    <UViewModal title="Mqtt客户端管理" :span="[8, 16]" v-model="editModel" :rules="rules" width="680px" @edit="gatewayEdit">
      <template #default="{model}">
        <URow :responsive="2">
          <UInputItem field="name" label="客户端名称" :maxlength="32" />
<!--          <UInputItem field="deviceSn" label="设备编号" :maxlength="32" />-->
          ...
          <a-divider v-if="protocolConfig && protocolConfig.length > 0" style="margin: 0px 0px 12px" dashed>Mqtt客户端配置(重连客户端后生效)</a-divider>
    ...
    <UViewTable :columns="columns" :scroll="{x: 1230}">
      ...
      <template #action="{record}">
        ...
        <UTag func="open" uid="child" :data="record" color="#2db7f5" v-auth="'iot:mqtt:child'">子设备</UTag>
        <UTag func="ajax" :disabled="record['connectType'] != 'Client'" :data="record" color="#55acee"
              v-auth="'iot:mqtt:connect'" :url="'/iot/device/connect/'+(record.status=='online' ? 'offline' : 'online')">
      ...
      <ULinkView uid="childView">
        <UViewSearch v-model="childSearchModel">
          <URow col="search">
            <UInputItem field="name" label="设备名称" />
            <UInputItem field="deviceSn" label="设备编号" />
            <UButton func="query" :url="`/iot/mqtt/view?pid=${editModel['id'] ? editModel['id'] : ''}`">搜索</UButton>
            <UButton func="reset">重置</UButton>
        ...
        <UViewTable :columns="childColumns" :scroll="{x: 1200}" >
          ...
          <template #action="{record}">
            <UFuncTag func="edit" :data="record" url="/iot/mqtt/edit">修改</UFuncTag>
            <UFuncTag func="del" :data="record" url="/iot/mqtt/del">删除</UFuncTag>
          </template>
        </UViewTable>
        <UViewModal title="子设备管理" :span="[8, 16]" v-model="childModel" :rules="childRules" width="680px" @edit="childEdit">
          <template #default="{model}">
            <URow :responsive="2">
              ...
              <UInputItem field="deviceSn" label="设备编号" :maxlength="32" />
              <USelectItem field="productId" label="所属产品" @change="productChange" v-model:source="productSource"
                           url="/iot/mqtt/products?deviceType=Child" labelField="name" valueField="id">
...
export default {
  ...
  setup() {
    ...
    let columns = ref([
      ...
      {field: "deviceSn", title: "客户端编号", width: 150},
